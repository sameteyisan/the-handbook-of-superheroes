  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          Center(child: CustomNetworkImage(url: hero.url).animate().flipH(delay: 200.ms)),
              ...
              CustomIconButton(
                ...
                icon: Obx(
                  () => AnimatedCrossFade(
                    ...
                    secondChild: const Icon(Ionicons.heart_outline, color: CColors.backgroundcolor),
                    crossFadeState:
          ...
          Obx(
            () =>
                controller.isLoading.value
                    ? const Padding(padding: EdgeInsets.only(top: 32), child: CenterLoading())
                    ...
                    : Column(
                      children: [
                        ...
                        ...controller.superhero.value!.biography.toMap().entries.map(
                          (kv) => DetailPanel(kv: kv),
                        ),
                        const Padding(
                          padding: EdgeInsets.symmetric(vertical: 16),
                          child: TitleWidget("Powerstats"),
                        ),
                        ...controller.superhero.value!.powerstats.toMap().entries.map(
                          (kv) => PowerStats(kv: kv),
                        ),
                        const Padding(
                          padding: EdgeInsets.symmetric(vertical: 16),
                        ...
                        ...controller.superhero.value!.appearance.toMap().entries.map(
                          (kv) => DetailPanel(kv: kv),
                        ),
                        const Padding(
                          padding: EdgeInsets.symmetric(vertical: 16),
                        ...
                        ...controller.superhero.value!.connections.toMap().entries.map(
                          (kv) => DetailPanel(kv: kv),
                        ),
                      ],
                    ),
